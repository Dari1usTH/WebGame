...
function showBoundaryWarning() {
  ...
  ctx.fillText('Apasă „Reîncepe” pentru a încerca din nou', canvas.width / 2, canvas.height / 2 + 35);
}
...
if (settingsBtn && settingsPop) {
  ...
  document.addEventListener('click', (e) => {
    ...
    const inside = settingsPop.contains(e.target) || settingsBtn.contains(e.target);
    if (!inside) {
...
function updateSwatches() {
  document.getElementById('penSwatch').style.backgroundColor = penColor;
  document.getElementById('bgSwatch').style.backgroundColor = bgColor;
  document.getElementById('starsSwatch').style.backgroundColor = starsColor;
}

function handleColorChange(type, color) {
  if (type === 'pen') {
    penColor = color;
    localStorage.setItem('circle_pen_color', color);
  } else if (type === 'bg') {
    bgColor = color;
    localStorage.setItem('circle_bg_color', color);
  } else if (type === 'stars') {
    starsColor = color;
    localStorage.setItem('circle_stars_color', color);
  }
  updateSwatches();
  reset();
}

if (penColorBtn) penColorBtn.addEventListener('click', () => openRGBPanel(penColor, (c) => handleColorChange('pen', c)));
if (bgColorBtn) bgColorBtn.addEventListener('click', () => openRGBPanel(bgColor, (c) => handleColorChange('bg', c)));
if (starsColorBtn) starsColorBtn.addEventListener('click', () => openRGBPanel(starsColor, (c) => handleColorChange('stars', c)));
